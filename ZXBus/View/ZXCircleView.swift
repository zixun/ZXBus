...
public class ZXCircleView: UIView,UIScrollViewDelegate {
    ...
    private func addTapGestureRecognizer() {
        ...
        tap.addTarget(self, action: #selector(ZXCircleView.tapHandler))
    }
    
    func tapHandler() {
        
        
        guard let respond = self.circleDelegate?.respondsToSelector(Selector("circleView:didSelectedCellAtIndex:")) else {
            return
        }
        
        if respond == true {
            self.circleDelegate!.circleView!(self, didSelectedCellAtIndex: self.dataIndexFromUIndex(self.currentUIndex))
        }
    }
}
